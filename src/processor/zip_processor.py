...
log = logging.getLogger('alto.zip_processor')

def extract_xml_pages_from_zip(zip_path: PathLike[str]) -> Iterator[bytes]:
    log.info(f"Open decade-zip file '{zip_path}'")
                    ...
                    for zipped_entity in opened_book.infolist():
                        if not zipped_entity.is_dir() and zipped_entity.filename.endswith('.xml'):
                            with opened_book.open(zipped_entity.filename, 'r') as f:
                                log.debug(f"Read xml(page) from {zipped_entity.filename}")
                                yield f.read()